...
function mandelbrot_gpu(width, height, max_iter, zoom, x_offset, y_offset)
...
function kernel_mandelbrot(pixels, width, height, max_iter, zoom, x_offset, y_offset, aspect_ratio)
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    j = (blockIdx().y - 1) * blockDim().y + threadIdx().y

    if i <= width && j <= height
        x0 = ((i - Float32(width) / 2) / (Float32(width) / 2)) * zoom * aspect_ratio + x_offset
        y0 = ((j - Float32(height) / 2) / (Float32(height) / 2)) * zoom + y_offset
        x = 0.0f0
        y = 0.0f0
        iter = 0
